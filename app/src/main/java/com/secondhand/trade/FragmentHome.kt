...
class FragmentHome : Fragment() {
    ...
    private fun initItemList() {
        ...
        // 날짜 기준 내림차순 정렬
        firestore.collection("board_test").orderBy("date", Query.Direction.DESCENDING).get().addOnSuccessListener { documents ->
            for (document in documents) {
                val dataItem = DataHome(
                ...
                homeAdapter.itemList.add(dataItem)
                homeAdapter.notifyDataSetChanged()
                binding.swipeHome.isRefreshing = false // 로딩 완료 후 인디케이터 숨기기
            }
